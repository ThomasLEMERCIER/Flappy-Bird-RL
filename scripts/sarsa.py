...
from src.utils.data_structures import SarsaParams, Transition
...
def train(env, agent: SarsaAgent, num_episodes, max_steps, visualize=False):
    """
    Train the agent.
...
def play_episode(agent: SarsaAgent, env, max_iter=1000, render=False):
    state, _ = env.reset()
    total_reward = 0
    done = False
...
if __name__ == '__main__':
    ...
    sarsa_params = SarsaParams(alpha=alpha, num_actions=env.action_space.n, gamma=gamma)
    agent = SarsaAgent(sarsa_params, EpsilonGreedy(epsilon=epsilon))
