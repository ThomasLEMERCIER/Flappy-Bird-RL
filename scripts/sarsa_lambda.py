...
from src.utils.data_structures import SarsaLambdaParams, Transition
...
def train(env, agent: SarsaLambdaAgent, num_episodes, max_steps, visualize=False):
    """
    Train the agent.
...
def play_episode(agent: SarsaLambdaAgent, env, max_iter=1000, render=False):
    state, _ = env.reset()
    total_reward = 0
    done = False
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train a SARSA agent to play Flappy Bird.')
    ...
    parser.add_argument('--l', type=float, default=0.9, help='Lambda parameter.')
    ...
    # initiate SARSA parameters
    sarsa_params = SarsaLambdaParams(alpha=alpha, num_actions=env.action_space.n, gamma=gamma, lambd=l)
    agent = SarsaLambdaAgent(sarsa_params, EpsilonGreedy(epsilon=epsilon))
